...
class GameFragment : Fragment() {
    ...
    private fun play() {
        ...
        if (currentRound != null) {
            ...
            val points = if (totalGameTime > 0) hits * 10f / (totalGameTime / 1000L) else 0f
            val resultIntent = Intent(requireActivity(), ResultActivity::class.java).apply {
                putExtra(EXTRA_POINTS, points)
                putExtra(EXTRA_SETTINGS, settings)
            }
            requireActivity().startActivity(resultIntent)
